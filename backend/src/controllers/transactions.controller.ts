...
export const uploadTransactionsCSV = async (req: Request, res: Response) => {
  try {
    ...
    const parsedTransactions: Transaction[] = records.map((record: any) => ({
      ...
      txnDate: new Date(record.txnDate || record["Txn Date"] || record.date || ""),
      txnType: record.txnType || record["Txn Type"] || "",
      description: record.description || record.Description || "",
      referenceNumber: record.referenceNumber || record["Reference Number"] || "",
      isSuspicious: String(record.isSuspicious || record["Is Suspicious"] || "").toLowerCase() === "true",
      flagLevel: record.flagLevel || record["Flag Level"] || "Low",
      flagReasons: record.flagReasons ? String(record.flagReasons).split(",") : [],
    }));
